  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 bg-white dark:bg-slate-900/60 border border-[#E5E7EB] dark:border-slate-800">
          <div className="w-1.5 h-1.5 rounded-full bg-accent-500 animate-pulse" />
          <Clock className="w-3.5 h-3.5 text-[#6B7280] dark:text-slate-500" />
          <span className="font-mono text-xs text-[#6B7280] dark:text-slate-500">LIVE</span>
        </div>
      </div>
          ...
          <TechnicalPanel key={stat.label} label={`STAT.${String(index + 1).padStart(2, '0')}`}>
            <div className="p-5">
              <div className="flex items-start justify-between mb-4">
                <stat.icon className="w-5 h-5 text-[#6B7280] dark:text-slate-600" />
                <div className="flex items-center gap-1 text-accent-500">
                  <TrendingUp className="w-3 h-3" />
                  <span className="font-mono text-xs">{stat.change}</span>
                </div>
              </div>
              <div className="font-mono text-4xl font-bold text-[#111827] dark:text-slate-100 tabular-nums">
                {mounted && <AnimatedCounter target={stat.value} />}
              </div>
              <div className="flex items-center justify-between mt-2">
                <span className="font-mono text-xs text-[#374151] dark:text-slate-500 tracking-wide">{stat.label}</span>
                <span className="font-mono text-[10px] text-[#6B7280] dark:text-slate-600">{stat.period}</span>
              </div>
            </div>
          </TechnicalPanel>
        ))}
      ...
      <section>
        <div className="flex items-center gap-3 mb-6">
          ...
          <h2 className="font-mono text-sm font-semibold text-[#111827] dark:text-slate-300 tracking-wide">
            QUICK ACTIONS
          </h2>
          <div className="flex-1 h-px bg-gradient-to-r from-[#E5E7EB] dark:from-slate-800 to-transparent" />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {quickActions.map((action) => (
            <Link
              key={action.href}
              href={action.href}
              className="group relative bg-white dark:bg-slate-900/40 border border-[#E5E7EB] dark:border-slate-800 hover:border-accent-500/50 hover:shadow-md transition-all duration-300"
              style={{
                clipPath: 'polygon(0 0, calc(100% - 12px) 0, 100% 12px, 100% 100%, 12px 100%, 0 calc(100% - 12px))',
              }}
            >
              {/* Corner cuts visual */}
              <div className="absolute top-0 right-0 w-3 h-3 border-l border-b border-[#E5E7EB] dark:border-slate-800 group-hover:border-accent-500/50 transition-colors" style={{ transform: 'translate(0, 0) rotate(45deg)', transformOrigin: 'top right' }} />

              <div className="p-4">
                <div className="flex items-center justify-between mb-3">
                  <div className="w-9 h-9 border border-[#E5E7EB] dark:border-slate-700 bg-[#F9FAFB] dark:bg-slate-800/50 flex items-center justify-center group-hover:border-accent-500/50 group-hover:bg-accent-500/10 transition-all">
                    <action.icon className="w-4 h-4 text-[#374151] dark:text-slate-400 group-hover:text-accent-500 transition-colors" />
                  </div>
                  <div className="flex items-center gap-1.5">
                    <span className="font-mono text-[10px] text-[#9CA3AF] dark:text-slate-700 group-hover:text-accent-500/70 transition-colors">
                      {action.shortcut}
                    </span>
                    <ArrowRight className="w-3.5 h-3.5 text-[#9CA3AF] dark:text-slate-700 group-hover:text-accent-500 group-hover:translate-x-0.5 transition-all" />
                  </div>
                </div>
                <h3 className="font-mono text-sm font-semibold text-[#111827] dark:text-slate-100 tracking-wide mb-1">
                  {action.title}
                </h3>
                <p className="text-xs text-[#374151] dark:text-slate-500 leading-relaxed line-clamp-2">
                  {action.description}
                </p>
              </div>
            </Link>
      ...
      <section>
        <div className="flex items-center justify-between mb-6">
          ...
          <Link
            href="/app/projects"
            className="font-mono text-xs text-[#6B7280] dark:text-slate-500 hover:text-accent-500 transition-colors flex items-center gap-1"
          >
            VIEW ALL
        ...
        <TechnicalPanel label="ACTIVITY.LOG">
          <div className="divide-y divide-[#E5E7EB] dark:divide-slate-800/50">
            {recentActivity.map((item, index) => (
              <div
                key={index}
                className="flex items-center justify-between px-5 py-4 hover:bg-[#F9FAFB] dark:hover:bg-slate-800/20 transition-colors"
              >
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    />
                    <span className="font-mono text-[10px] text-[#9CA3AF] dark:text-slate-600 w-16">
                      {String(index + 1).padStart(2, '0')}.LOG
                    </span>
                  </div>
                  <div>
                    <p className="text-sm text-[#111827] dark:text-slate-300">
                      <span className="text-[#6B7280] dark:text-slate-500">{item.action}:</span>{" "}
                ...
                <div className="flex items-center gap-4">
                  ...
                  <span className="font-mono text-xs text-[#6B7280] dark:text-slate-600 w-12 text-right">{item.time}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="p-6">
          <div className="flex items-start gap-5">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="font-mono text-base font-semibold text-[#111827] dark:text-slate-100 tracking-wide mb-2">
                GETTING STARTED
              </h3>
              <p className="text-sm text-[#374151] dark:text-slate-500 mb-5 leading-relaxed">
              ...
              <div className="flex items-center gap-3">
                ...
                <Link
                  href="/app/stackup"
                  className="inline-flex items-center gap-2 px-4 py-2 border border-[#E5E7EB] dark:border-slate-700 text-[#374151] dark:text-slate-300 font-mono text-xs font-medium hover:border-[#D1D5DB] dark:hover:border-slate-600 hover:text-[#111827] dark:hover:text-slate-100 transition-colors"
                >
