...
- AI Orchestration (Extraction, Interpretation, Combined, QA agents; `/api/fcf/interpret` orchestrator).
...
| Sprint 3 | AI vertical slice for JSON inputs; interpreter route usable | AI Orchestration; Core Features | `/api/ai/*` endpoints + prompts; `/api/fcf/interpret` orchestrator; `/app/interpreter` JSON load/validate + explanation; CalcResults UI |
...
| Sprint 5 | Projects/measurements completion; hardening, perf, security | Projects & Settings; Testing & Security | Measurement runs capture; RLS and signed URL tests; performance checks (preview latency, `/api/fcf/interpret` P90); QA confidence surfacing; accessibility pass |
...
- M3 (Sprint 3 mid/end): AI orchestration slice working for JSON input via `/app/interpreter` and `/api/fcf/interpret`.
- M4 (Sprint 4 end): Builder + image interpreter + exports functional; settings and project shell present.
- M5 (Sprint 5 end): Projects/measurements fully wired; performance/security targets validated in staging; QA confidence UX live.
...
- Build deterministic core first (schema, rules, calculators, units) to anchor AI prompts and QA checks; prevents AI drift.
